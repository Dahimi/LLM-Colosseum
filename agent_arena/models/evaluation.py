...
class Evaluation(BaseModel):
    ...
    evaluation_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique evaluation identifier")
    match_id: str = Field(description="ID of the match being evaluated")
    judge_id: str = Field(description="ID of the judging agent")
    
    # Timing
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Evaluation creation timestamp")
    submitted_at: Optional[datetime] = Field(default=None, description="Evaluation submission timestamp")
    
    # Scores for each agent
    agent1_scores: List[JudgeScore] = Field(default_factory=list, description="Scores for agent 1")
    agent2_scores: List[JudgeScore] = Field(default_factory=list, description="Scores for agent 2")
    
    # Overall assessment
    agent1_total_score: float = Field(default=0.0, description="Total weighted score for agent 1")
    agent2_total_score: float = Field(default=0.0, description="Total weighted score for agent 2")
    recommended_winner: Optional[str] = Field(default=None, description="Judge's recommended winner (agent ID)")
    
    # Judge's reasoning
    overall_reasoning: str = Field(default="", description="Judge's overall reasoning for the decision")
    comparative_analysis: str = Field(default="", description="Comparative analysis of both responses")
    key_differentiators: List[str] = Field(default_factory=list, description="Key factors that differentiated the responses")
    
    # Quality metrics
    evaluation_quality: float = Field(default=0.0, description="Quality of this evaluation (meta-score)")
    is_final: bool = Field(default=False, description="Whether this evaluation is finalized")
    
    # Metadata
    evaluation_time_seconds: float = Field(default=0.0, description="Time taken to complete evaluation")
    ...
    def add_score(self, agent_id: str, criterion: EvaluationCriteria, score: float, reasoning: str, confidence: float = 1.0) -> None:
    ...
    def calculate_total_scores(self, criteria_weights: Optional[Dict[EvaluationCriteria, float]] = None) -> None:
    ...
    def finalize_evaluation(self, overall_reasoning: str = "", comparative_analysis: str = "") -> None:
        ...
        if self.created_at:
            self.evaluation_time_seconds = (self.submitted_at - self.created_at).total_seconds()
    
    def get_score_by_criterion(self, agent_id: str, criterion: EvaluationCriteria) -> Optional[JudgeScore]:
    ...
    def get_agent_scores_summary(self, agent_id: str) -> Dict[str, Any]:
        ...
        scores = self.agent1_scores if agent_id.endswith("1") else self.agent2_scores
        total_score = self.agent1_total_score if agent_id.endswith("1") else self.agent2_total_score
        ...
        return {
            ...
            "average_score": sum(s.score for s in scores) / len(scores) if scores else 0.0,
            "average_confidence": sum(s.confidence for s in scores) / len(scores) if scores else 0.0
        }
