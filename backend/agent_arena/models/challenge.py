...
class Challenge(BaseModel):
    ...
    challenge_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique challenge identifier")
    ...
    creator_id: Optional[str] = Field(default=None, description="ID of agent who created this challenge")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Creation timestamp")
    
    # Challenge parameters
    time_limit_minutes: Optional[int] = Field(default=None, description="Time limit in minutes (None = no limit)")
    max_response_length: Optional[int] = Field(default=None, description="Maximum response length in characters")
    requires_structured_output: bool = Field(default=False, description="Whether response must follow a specific format")
    output_schema: Optional[Dict[str, Any]] = Field(default=None, description="Pydantic schema for structured output")
    ...
    expected_concepts: List[str] = Field(default_factory=list, description="Concepts expected in a good response")
    scoring_rubric: Dict[str, Any] = Field(default_factory=dict, description="Detailed scoring criteria")
    
    # Usage tracking
    times_used: int = Field(default=0, description="Number of times this challenge has been used")
    average_score: float = Field(default=0.0, description="Average score across all attempts")
    difficulty_rating: float = Field(default=0.0, description="Actual difficulty based on performance (0-10)")
    discrimination_power: float = Field(default=0.0, description="How well it separates strong/weak agents (0-1)")
    
    # Metadata
    tags: List[str] = Field(default_factory=list, description="Tags for categorization")
    source: str = Field(default="generated", description="Source of the challenge (generated, imported, etc.)")
    ...
    def get_prompt(self, include_hints: bool = False, hint_level: int = 0) -> str:
        ...
        if include_hints and self.hints and hint_level > 0:
            ...
            prompt_parts.extend([
                "**Hints:**",
                *[f"- {hint}" for hint in available_hints],
                ""
            ])
        ...
        if self.max_response_length:
            prompt_parts.append(f"**Max Response Length:** {self.max_response_length} characters")
    ...
    def update_stats(self, score: float, agent_elo: float) -> None:
        ...
        if self.times_used == 1:
            ...
            self.average_score = ((self.average_score * (self.times_used - 1)) + score) / self.times_used
        ...
        expected_performance = min(10.0, max(0.0, agent_elo / 200))  # Normalize ELO to 0-10 scale
        ...
        if performance_diff < 0:  # Agent performed worse than expected
            self.difficulty_rating = self.difficulty_rating * (1 - alpha) + (self.difficulty_rating + abs(performance_diff)) * alpha
        else:  # Agent performed better than expected
            self.difficulty_rating = max(0.0, self.difficulty_rating * (1 - alpha) + (self.difficulty_rating - performance_diff) * alpha)
